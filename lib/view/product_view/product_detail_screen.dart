...
class _ProductDetailScreenState extends State<ProductDetailScreen> with SingleTickerProviderStateMixin {
      ...
      actions: [
              ...
              (userProfile != null && _isProductSaved(userProfile!, widget.productId)) ? Icons.bookmark : Icons.bookmark_border,
                ...
                try {
                  ...
                  if (mounted) {
                    ...
                    _handleSubScreenNavigation(ProfileScreen(userProfile));
                  }
        ...
        _buildCircleIcon(Icons.add_shopping_cart_outlined, () async {
          if (product.variants.isNotEmpty && _selectedVariantIndex == -1) {
            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Please select a color/size first"), backgroundColor: Colors.red));
            return;
          }
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Adding to cart..."), duration: Duration(seconds: 1)));
          try {
            ...
            if (mounted) {
              ...
              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Successfully added to Cart!"), backgroundColor: Colors.green));
              _handleSubScreenNavigation(const CartScreen());
            }
          } catch (e) { if (mounted) { ScaffoldMessenger.of(context).hideCurrentSnackBar(); ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(e.toString()), backgroundColor: Colors.red)); } }
        }),
      ...
      flexibleSpace: FlexibleSpaceBar(
        background: Stack(
          children: [
            ...
            if (product.images.length > 1) Positioned(bottom: 16, left: 0, right: 0, child: Center(child: SmoothPageIndicator(controller: _pageController, count: product.images.length))),
          ],
        ),
      ),
    );
  ...
  Widget _buildTitlePriceAndRating() {
    return Row(
      ...
      children: [
              ...
              Row(
                children: [
                  ...
                  InkWell(
                    ...
                    child: Text.rich(TextSpan(children: [TextSpan(text: '${product.averageRating?.toStringAsFixed(1) ?? '0.0'} ', style: const TextStyle(color: Color(0xFF787676), fontSize: 12)), TextSpan(text: '(${product.reviews.length} reviews)', style: const TextStyle(color: Color(0xFF347EFB), fontSize: 12, decoration: TextDecoration.underline))])),
                  ),
                ],
              ),
            ],
        ...
        Padding(padding: const EdgeInsets.only(left: 16.0), child: Text('${product.price} ₹', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: Color(0xFF292526)))),
      ],
    );
  }
            ...
            child: SizedBox(
              ...
              child: TextField(
                ...
                decoration: InputDecoration(
                  ...
                  suffixIcon: Padding(padding: const EdgeInsets.all(7.0), child: GestureDetector(onTap: () { print("Comment submitted: ${_commentController.text}"); _commentController.clear(); FocusScope.of(context).unfocus(); }, child: const CircleAvatar(radius: 19, backgroundColor: Color(0xFFA2DC00), child: Icon(Icons.arrow_forward, color: Colors.white, size: 20)))),
                ),
              ),
            ),
          ),
  ...
  Widget _buildBottomActionBar() {
    return Container(
      padding: const EdgeInsets.fromLTRB(24, 12, 24, 24), decoration: BoxDecoration(color: Colors.white, boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.1), blurRadius: 10)]),
      child: Row(
        children: [
          Expanded(child: OutlinedButton(onPressed: () { _handleSubScreenNavigation(ZatchingDetailsScreen(zatch: Zatch(id: "temp1", name: product.name, description: product?.description ?? "", seller: "Seller Name", imageUrl: product.images.isNotEmpty ? product.images.first.url : '', active: true, status: "My Offer", quotePrice: "${product.price.toStringAsFixed(0)} ₹", sellerPrice: "${product.price.toStringAsFixed(0)} ₹", quantity: 1, subTotal: "${(product.price * 1).toStringAsFixed(0)} ₹", date: DateTime.now().toString()))); }, style: OutlinedButton.styleFrom(padding: const EdgeInsets.symmetric(vertical: 16), side: const BorderSide(color: Color(0xFFCCF656)), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20))), child: const Text('Zatch', style: TextStyle(color: Colors.black, fontSize: 16)))),
          const SizedBox(width: 12),
          Expanded(
            child: ElevatedButton(
              onPressed: () {
                if (product.variants.isNotEmpty && _selectedVariantIndex == -1) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Please select a color/size"))); return; }
                ...
                final itemToPurchase = cart_model.CartItemModel(id: "temp_${product.id}", productId: product.id, name: product.name, description: product.description, price: product.price.toDouble(), discountedPrice: product.price.toDouble(), image: product.images.isNotEmpty ? product.images.first.url : "", images: product.images.map((img) => cart_model.ImageModel(id: img.id, publicId: img.publicId, url: img.url)).toList(), variant: cart_model.VariantModel(color: selectedShade, size: selectedSize), selectedVariant: {"color": selectedShade, "size": selectedSize}, quantity: 1, category: product.category ?? "", subCategory: product.subCategory ?? "", lineTotal: product.price.toInt());
                _handleSubScreenNavigation(CheckoutOrPaymentsScreen(isCheckout: true, selectedItems: [itemToPurchase], itemsTotalPrice: product.price, shippingFee: 10.0, subTotalPrice: product.price + 10.0));
              },
  ...
  bool _isProductSaved(UserProfileResponse profile, String productId) => false;
}
              ...
              child: GestureDetector(
                onTap: () {
                  if (_replyController.text.isNotEmpty) {
                    print("Replying to $userName: ${_replyController.text}");
