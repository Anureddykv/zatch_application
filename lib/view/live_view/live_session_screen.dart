...
import 'package:zatch/view/SidebarItem.dart';
...
class _LiveSessionScreenState extends State<LiveSessionScreen> {
  ...
  Future<void> _onShareTap() async {
    try {
      final shareData = await ApiService().shareLiveSession(widget.sessionId);
      if (shareData.success) {
        await Share.share(
          shareData.shareText.primary,
          subject: shareData.shareText.title,
        );
      } else {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(shareData.message.isNotEmpty ? shareData.message : "Failed to generate share link")),
          );
        }
      }
    } catch (e) {
                        ...
                        child: Align(
                          ...
                          child: Row(
                            ...
                            children: [
                              Expanded(
                                child: Padding(
                                  padding: const EdgeInsets.only(left: 16.0, right: 10.0, bottom: 10),
                                  child: SizedBox(
                                    height: 250,
                                    child: ShaderMask(
                                      ...
                                      child: ListView.builder(
                                        ...
                                        itemBuilder: (context, index) => _buildChatMessage(_comments[index]),
                                      ),
                                    ),
                                  ),
                                ),
                              ...
                              Padding(
                                padding: const EdgeInsets.only(right: 16.0, bottom: 20, left: 10),
  ...
  Widget _buildFigmaProductCard(Product product) {
