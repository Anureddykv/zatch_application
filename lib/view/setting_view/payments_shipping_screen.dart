...
import 'package:zatch/view/coupon_apply_screen.dart';
import 'package:zatch/view/home_page.dart';
import 'package:zatch/view/order_view/order_place_screen.dart';
import 'package:zatch/view/setting_view/add_new_address_screen.dart';
import 'package:zatch/view/setting_view/payment_method_screen.dart';
...
class _CheckoutOrPaymentsScreenState extends State<CheckoutOrPaymentsScreen> {
  ...
  Future<void> _deleteAddress(String addressId) async {
    try { await _apiService.deleteAddress(addressId); _fetchInitialData(); } catch (e) { SnackBarUtils.showTopSnackBar(context, "Failed to delete address: $e", isError: true); }
  }
            ...
            if (widget.isCheckout)
              Container(
                ...
                child: ElevatedButton(
                  onPressed: () async {
                    if (_addresses.isEmpty) { SnackBarUtils.showTopSnackBar(context, "Please add and select a shipping address.", isError: true); return; }
                    if (_selectedPaymentMethod == null) { SnackBarUtils.showTopSnackBar(context, "Please select a payment method.", isError: true); return; }
                    final selectedAddress = _addresses[selectedAddressIndex];
                    setState(() => isLoading = true);
                    try {
                      Map<String, dynamic> response;
                      if (widget.isDirectOrder) {
                        // Exact mapping for Direct Order request body
                        final List<Map<String, dynamic>> directItems = widget.selectedItems?.map((item) {
                          return { 
                            'productId': item.productId, 
                            'variantColor': item.variant.color ?? item.variant.shade,
                            'qty': item.quantity,
                          };
                        }).toList() ?? [];

                        response = await _apiService.createDirectOrder(
                          addressId: selectedAddress.id, 
                          paymentMethod: "cod", 
                          buyerNote: "Please deliver after 6pm", 
                          items: directItems
                        );
                      } else {
                        final List<Map<String, dynamic>> cartItems = widget.selectedItems?.map((item) {
                    ...
                    } catch (e) { if (mounted) SnackBarUtils.showTopSnackBar(context, "Failed to place order: $e", isError: true); } finally { if (mounted) setState(() => isLoading = false); }
                  },
