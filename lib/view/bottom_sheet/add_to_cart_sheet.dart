...
class _AddToCartSheetState extends State<AddToCartSheet> {
  ...
  Future<void> _handleAddToCart() async {
    ...
    try {
      ...
      // 1. Close the AddToCart Sheet first
      Navigator.pop(context);

      // 2. Use the onNavigate callback to open CartScreen as a sub-screen
      // This allows the Bottom Nav to remain visible if HomePage is handling it.
      WidgetsBinding.instance.addPostFrameCallback((_) {
        // Show success message
        SnackBarUtils.showTopSnackBar(
          ApiService.navigatorKey.currentContext!, 
      ...
      if (!mounted) return;
      SnackBarUtils.showTopSnackBar(context, "Failed to add to cart: ${e.toString()}", isError: true);
    } finally {
      if (mounted) {
        setState(() {
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    final double sellingPrice = widget.product.discountedPrice ?? widget.product.price;
    final double subTotal = sellingPrice * quantity;
